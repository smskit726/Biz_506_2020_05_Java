...
public class VarBoolean_04 {
	...
	public static void main(String[] args) {
		 ...
		 * 선언만 된 변수는 메모리(기억장치)의 어딘가에 지정된 위치인데 알 수 없는 어떤 값이 저장되어 있을수도 있다.
		 * 때문에 변수는 어떠한 값을 할당, 저장, 대입 하기 전까지는 읽기를 할 수 없다.
		 * 
		 *  변수는 선언을 한 후 최소한 1번은 값을 저장해주어야만 읽을 수가 있다.
		 *  이러한 것을 변수가 사용가능하다  라고 한다.
		 *  사용가능하다 = 저장, 읽기 가 모두 가능한 상태
		 *  
		 *  변수를 선언한 후 어떠한 값을 저장하는 것을 변수를 초기화한다 라고 한다.
		 *  변수는 초기화가 되어야만 사용(읽기, 쓰기)이 가능하다.
		 *  
		 *  변수는 기본적으로 제공되는 int, float, long, double, boolean, char... 들은 
		 ...
		 * 기본적으로 제공되지 않는 필요에 따라 JDK를 import 해서 선언하는 변수들은 초기화하는 방법이 조금 다르다.
		 * 
		 * 키워드 변수 = new 키워드();
		 ...
		 * 
		 *.nextInt(100) method를 연결하면 이 명령문은 0~99까지의 임의의 정수를 만들어 num1 변수에 저장한다.
		 */
