...
public class ScoreEx_03 {
	public static void main(String[] args) {
		...
		// 초기화를 할 때는 new ArrayList()를 사용해서 생성을 한다.
		// str1과 int1은 개수가 정해지지 않은 특별한 배열과 같은 존재가 된다.
		// List type의 인스턴스, List type의 객체 라고 한다
		
		// Generic : <> 들어가는 키워드 문법형식
		// 리스트의 type을 결정짓는 키워드 문법형식
		...
		// add() method를 호출해서 데이터를 매개변수로 전달하면 그때마다 size가 1씩 증가하는 리스트로 변경된다.
		...
		// java 1.8 이상에서는 초기화코드(new ArrayList)에는 Generic을 생략할 수 있다.
		List<ScoreVO> score18 = new ArrayList<>();
		
		// 개발자가 작성한 ScoreVO 클래스의 type의 인스턴스를 포함할 수 있는 리스트
