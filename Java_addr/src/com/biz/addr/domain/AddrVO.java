 ...
 *  매개변수가 없는 기본생성자와 필드변수 리스트를 매개변수로 받아서
 *  필드변수 값을 지정하는 필드생성자를 만들 수 있다.
 *  
 */
...
public class AddrVO {
	...
	/*
	 * 필드변수가 String를 포함한 primitive(기본형) 변수형 일때는 초기화가 자동으로 이루어진다.
	 * 문자열은 "" 으로 숫자는 0으로
	 * 
	 * - Primitive(기본형) 변수형 -
	 ...
	 * 	Byte, Short, Long, Integer, Float, Double, Boolean, Character
	 * 
	...
	/*
	 * 필드(변수) 생성자
	 * 객체를 초기화할 때 필수적으로 필요한 이름과 전화번호 값만` 저장하여 주소를 만들고 싶다.
	 * 이 때, 호출하여 사용하는 생성자
	 */
	...
	// 객체생성될 때 필드변수들에게 특별히 값을 만들어주고 싶을 때, 임의로 기본 생성자를 만들고 필드변수들에 값을 Setting 할 수 있다.
	public AddrVO() {
