...
public class AddrService02 {
	...
	// Scanner 와 같은 유형의 클래스는 System의 resource를 사용하므로 신중하게 사용하는 것이 좋다.
	// resource 클래스의 경우는 가급적 필드영역에서 직접 초기화 하지 말고
	// 생성자 method에서 초기화를 시키는 것이 좋다.
	// 필드영역에서 초기화된 객체들은 현재 클래스의 인스턴스가 만료되어도
	// GC(Garbage Collection)이 작동될 때까지 메모리에 남아 있는 경우가 있다.
	private Scanner scan; // = new Scanner(System.in);
	
	// 생성자에서 초기화된 객체는 현재 클래스의 인스턴스의 수명과 같다.
	// 즉, 현재 클래스를 사용하여 생성된 인스턴스가 사용이 만료되면 생성자에서 초기화된 객체들도 같이 만료(소멸)된다.
