...
public class AddrService01 {
	 ...
	 * 
	 * 아래의 1 ~ 3의 방법으로 반드시 변수를 초기화 해주어야 한다. (교재 p.153)
	 * 1. 필드변수를 선언과 동시에 초기화 하기
	 * (메모리 관리상 여러가지 문제를 일으킬수 있기 때문에 가급적 사용하지 말자
	 * 
	 * 2. 생성자를 이용하여 초기화 하기
	 * 3. 사용하기 직전에 초기화 하기
	 * 
	...
	// 누군가 AddrService01 클래스를 객체로 선언하고 초기화를 하기 위해 생성자를 호출하면
	// 자동으로 addrList 필드변수가 초기화 되어 이후의 코드에서 NullpointerException이 발생하는 것을 방지한다.
	...
	public void makeAddr() {
		...
		// 필드변수를 사용하기 직전에 초기화 하는 방식은 유지보수가 어려워지는 코드가 된다.
		addrList = new ArrayList<AddrVO>();
		
		// 1명의 주소를 저장할 VO 객체(인스턴스)
		// AddrVO 클래스를 사용하여 addrVO 객체(인스턴스)를 선언 생성자 method를 호출하여 객체를 초기화(사용가능하도록) 한다.
		...
		*/
		...
		addrVO = new AddrVO(); 
		addrVO.setName("클레오");
		addrVO.setTel("010-5555-5555");
		addrVO.setAddr("대전");
		addrVO.setAge(32);
		addrVO.setNet("형");
		addrList.add(addrVO);
		
		// 많은 데이터를 처리 할 때 필드변수 생성자를 이용한다.
		addrVO = new AddrVO("성춘향", "010-3333-3333", "남원시", 16, "이몽룡연인");
		addrList.add(addrVO);
		...
		addrVO = new AddrVO("장보고", "010-4444-4444");
		addrVO.setAge(30);
		addrList.add(addrVO);
	}
	...
	public void addrList() {
		...
		// i<5 로 설정을 하게 되면 5명의 주소록을 표시할때는 문제가 없으나 만약 주소록 데이터의 개수가 변경되면 문제를 일으킨다.
		// addrList의 개수를 별도의 변수에 담아 사용한다.
		int addrSize = addrList.size();
		for(int i = 0; i<addrSize; i++) {
			AddrVO addr = addrList.get(i);
			System.out.print(addr.getName()+"\t");
			System.out.print(addr.getTel()+"\t");
			System.out.print(addr.getAddr()+"\t");
			System.out.print(addr.getAge()+"\t");
			System.out.print(addr.getNet()+"\n");
			}
		System.out.println(line.do_line(lineLength));
	}
