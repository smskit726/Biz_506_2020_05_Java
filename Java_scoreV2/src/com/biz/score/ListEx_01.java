...
public class ListEx_01 {
	public static void main(String[] args) {
		...
		List<String> strList;  // String 객체(문자열)이 요소로 포함된 리스트이다.
		...
		// for문이 strList 개수만큼 반복 수행되는 동안 계속해서 size() method를 호출하여 연산을 수행하도록 한다.
		// strList 개수가 for 반복문 내에서 변경되지 않는 경우 매우 불필요한 연산을 반복 수행하는 결과가 된다.
		for(int i = 0; i <strList.size(); i++) {
			System.out.println(strList.get(i));
		}
		
		// 개수를 1번만 조회하고 for 반복문 수행하기
		// 이 코드는 개수를 1번만 조회하고 for 반복문을 수행하기 때문에 위으 코드보다는 효율적이다.
		// 하지만 개수를 조회하는 코드를 생략하면 혹시 리스트 개수가 변경되었을 시
