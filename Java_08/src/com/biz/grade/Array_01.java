...
public class Array_01 {
	public static void main(String[] args) {
		...
		// integer형 배열 20개씩을 선언 : 20개의 저장공간을 가진 변수를
		...
		/*
		 * 위에서 생성한 4개의 배열에 각각 20개의 점수를 할당하기 위해 배열의 개수만큼
		 * 반복문을 실행하고, 반복문이 실행되면서 만들어지는 i 변수의 값을 각 배열의 index로 주입하여
		 * 배열[0] ~ 배열[19]까지의 저장공간에 값을 할당하기 위해 시작하는 반복문 코드
		 * intKor[0] ~ intKor[19], intEng[0] ~ intEng[19] ...
		 * 이러한 위치가 지정된 변수에 임의 정수 값을 생성하여 할당한다.
		 * 성적처리를 테스트하기 위한 점수를 만들어서 배열에 저장하는 코드
		 */
		for(int i=0; i<20; i++) {
		...
		*/
		...
		for(int i=0; i<20; i++) {
			System.out.printf("%d\t\t%d\t\t%d\t\t%d\t\t%d\t\t%d\n", intKor[i], intEng[i],intMath[i],intMusic[i],intStSum[i],intStSum[i]/4);
